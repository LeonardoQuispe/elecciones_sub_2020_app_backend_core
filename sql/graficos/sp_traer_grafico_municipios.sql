drop function if exists sp_traer_grafico_municipios;
CREATE OR REPLACE FUNCTION sp_traer_grafico_municipios(
	_id_usuario bigint, 
...
begin
	...
	sql = 'select 
			...
			inner join recinto r on r.id_municipio = m.id
			inner join mesa me on me.id_recinto = r.id
			left join conteo c on c.id_mesa  = me.id and c.id_tipo_conteo=' || _id_tipo_conteo || ' and c.estado = ''AC''  and me.id_estado_mesa  = 2
	...
	sql = sql || '	group by m.nombre, p.sigla, m.id, dmp.id, p.id, p.color, m.escanos ';
	sql = sql || '  order by m.id, dmp.id';
